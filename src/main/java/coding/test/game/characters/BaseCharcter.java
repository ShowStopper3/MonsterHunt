...
/**
 * 
 * superclass for all charcter (includes player, monsters...)
 *
 * @author ssachdev
 *
 */
public abstract class BaseCharcter {
	protected ItemRepository itemRepo = RepositoryServices.getItemRepository();
	...
	public BaseCharcter(int healthMax, int health, String name, int gold, Storage storage,
	...
	public void printEquipment() {
		QueueServices.offer("\n------------------------------------------------------------");
		QueueServices.offer("Equipped Items:");
		if (equipment.keySet().size() == 0) {
			QueueServices.offer("--Empty--");
		} else {
			int i = 0;
			...
			for (Map.Entry<EquipmentLocation, Item> item : equipment.entrySet()) {
				if (item.getKey() != null && !hands.equals(item.getValue()) && item.getValue() != null) {
					QueueServices.offer(locations.get(item.getKey()) + " - " + item.getValue().getName());
				} else {
					i++;
				}
			}
