...
/**
 * This class will help in transition of character from one level to another
 * 
 * @author ssachdev
 *
 */
public class CharcterServices {
	public void checkForCharcterChange(Player player, String triggerType, String keyword) {
			...
			if (json.has(currentCharacter)) {
				...
				if (currentCharacterTransitions.has(triggerType)) {
					...
					if (events.has(keyword)) {
						...
						QueueServices.offer("Warning: The effects for the '" + triggerType + "' event and the '"
								+ currentCharacter + "' character was not found");
					}
				} else {
					QueueServices.offer("Warning: The event '" + triggerType + "' for the '" + currentCharacter
							+ "' character was not found");
				}
			} else {
				QueueServices.offer("Warning: The character '" + currentCharacter + "' was not found");
			}
	...
	public void checkForCharacterChange(Player player) {
		...
		if (!highestCharacter.equals(currentCharacter)) {
			...
			QueueServices.offer("You're character type is now changed! You are now a " + highestCharacter + "!");
		}
